...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const search = q ? { $or: [{ name: { $regex: q.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"), $options: "i" } }, { phone: { $regex: q.replace(/\D/g, ""), $options: "i" } }, { email: { $regex: q.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"), $options: "i" } }] } : {};
    const access=["counsellor","manager"].includes(session.role)?{$or:[{assignedTo:session.userId},{followUpAssignedTo:session.userId},{counsellor:session.name},{followUpAssignee:session.name}]}:{};
    const leads = await Lead.find({ $and:[search,access,{archivedAt:null}] }).sort({ updatedAt: -1 }).limit(200).lean();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ownership=["counsellor","manager"].includes(session.role)?{assignedTo:session.userId,counsellor:session.name}:{};
    const lead = await Lead.create({ ...parsed.data, ...ownership, tags, phone, email, createdBy: session.userId, activities: [{ type: "note", text: "Lead profile created", authorId: session.userId, authorName: session.name }] });
