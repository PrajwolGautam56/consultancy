...
export default function Home() {
  ...
  const filtered = useMemo(() => leads.filter(l => `${l.name} ${l.phone} ${l.email} ${l.country} ${l.course} ${l.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase()) && (stageFilter==="All stages"||l.stage===stageFilter) && (counsellorFilter==="All counsellors"||l.counsellor===counsellorFilter) && (tagFilter==="All tags"||l.tags.includes(tagFilter))), [leads, query,stageFilter,counsellorFilter,tagFilter]);
  ...
  return (
    <main className="app-shell">
      ...
      {selected && <Profile lead={selected} counsellors={counsellors} allTags={allTags} close={()=>setSelected(null)} update={persistLead}/>}
    </main>
  );
}
...
function Profile({lead,counsellors,allTags,close,update}:{lead:Lead;counsellors:string[];allTags:string[];close:()=>void;update:(l:Lead,activity?:{type:string;text:string})=>void}) {
