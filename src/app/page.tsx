...
type Lead = {
  ...
  education: string; passedOutInstitute:string; country: string; course: string; university: string;
  source: string; stage: Stage; counsellor: string; updated: string;
  nextFollowUp: string; nextFollowUpISO?:string|null; priority: "High" | "Medium" | "Low"; tags:string[]; inOffice?: boolean; activities?:Activity[];
};
...
function mapLead(raw:Record<string,unknown>):Lead {
  ...
  return { id:String(raw._id),name:String(raw.name||""),phone:String(raw.phone||""),email:String(raw.email||""),address:String(raw.address||""),education:String(raw.education||""),passedOutInstitute:String(raw.passedOutInstitute||""),country:String(raw.country||""),course:String(raw.course||""),university:String(raw.university||""),source:String(raw.source||"Other"),stage:(raw.stage||"New inquiry") as Stage,counsellor:String(raw.counsellor||"Unassigned"),updated:raw.updatedAt?date(raw.updatedAt):"Just now",nextFollowUp:date(raw.nextFollowUp),nextFollowUpISO:raw.nextFollowUp?String(raw.nextFollowUp):undefined,priority:(raw.priority||"Medium") as Lead["priority"],tags:Array.isArray(raw.tags)?raw.tags.map(String):[],inOffice:Boolean(raw.inOffice),activities:(raw.activities||[]) as Activity[] };
}
...
export default function Home() {
  ...
  const filtered = useMemo(() => leads.filter(l => `${l.name} ${l.phone} ${l.email} ${l.country} ${l.course} ${l.passedOutInstitute} ${l.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase()) && (stageFilter==="All stages"||l.stage===stageFilter) && (counsellorFilter==="All counsellors"||l.counsellor===counsellorFilter) && (tagFilter==="All tags"||l.tags.includes(tagFilter))), [leads, query,stageFilter,counsellorFilter,tagFilter]);
  ...
  const allTags=useMemo(()=>Array.from(new Set(leads.flatMap(l=>l.tags))).sort((a,b)=>a.localeCompare(b)),[leads]);
  const allInstitutes=useMemo(()=>Array.from(new Set(leads.map(l=>l.passedOutInstitute.trim()).filter(Boolean))).sort((a,b)=>a.localeCompare(b)),[leads]);
  ...
  async function persistLead(changed:Lead,activity?:{type:string;text:string}) {
    ...
    if(result.emailStatus==="failed")setAppError("Visitor was checked out, but the thank-you email could not be sent.");
  }
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        <header className="topbar"><button className="menu" aria-label="Open navigation" onClick={() => setMobileNav(true)}><Menu/></button><div className="global-search"><Search size={18}/><input ref={searchRef} value={query} onChange={e => setQuery(e.target.value)} placeholder="Search name, phone, email..."/><kbd>⌘ K</kbd></div><div className="notification-wrap"><button className="icon-btn" onClick={()=>setNotificationsOpen(!notificationsOpen)} aria-label="Notifications"><Bell size={20}/>{dueFollowUps.length>0&&<i/>}</button>{notificationsOpen&&<div className="notification-popover"><div><strong>Follow-up reminders</strong><button onClick={()=>setNotificationsOpen(false)} aria-label="Close notifications"><X size={15}/></button></div>{dueFollowUps.slice(0,5).map(lead=><button key={lead.id} onClick={()=>{setSelected(lead);setNotificationsOpen(false)}}><CalendarClock size={16}/><span><strong>{lead.name}</strong><small>{lead.nextFollowUp}</small></span></button>)}{dueFollowUps.length===0&&<p>No scheduled reminders.</p>}<button className="view-reminders" onClick={()=>{setActive("Follow-ups");setNotificationsOpen(false)}}>View all follow-ups</button></div>}</div><button className="primary" aria-label="Add new lead" onClick={openLeadForm}><Plus size={18}/> Add new lead</button></header>
        ...
        <div className="content">
          ...
          </> : active === "Office visitors" ? <OfficeView leads={leads} update={persistLead} select={setSelected} addNew={openLeadForm}/> : active === "Team" ? <TeamView/> : active === "Follow-ups" ? <FollowUpsView leads={leads} select={setSelected} update={persistLead}/> : active === "Settings" ? <SettingsView logout={logout}/> : <section className="panel directory"><div className="directory-tools"><div className="global-search inner"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search records..."/></div><select value={stageFilter} onChange={e=>setStageFilter(e.target.value)}><option>All stages</option>{Object.keys(stageStyle).map(s=><option key={s}>{s}</option>)}</select><select value={counsellorFilter} onChange={e=>setCounsellorFilter(e.target.value)}><option>All counsellors</option>{counsellors.map(name=><option key={name}>{name}</option>)}</select><select value={tagFilter} onChange={e=>setTagFilter(e.target.value)}><option>All tags</option>{allTags.map(tag=><option key={tag}>{tag}</option>)}</select>{(stageFilter!=="All stages"||counsellorFilter!=="All counsellors"||tagFilter!=="All tags"||query)&&<button className="clear-filters" onClick={()=>{setStageFilter("All stages");setCounsellorFilter("All counsellors");setTagFilter("All tags");setQuery("")}}><X size={14}/> Clear</button>}</div><LeadTable leads={filtered} select={setSelected} update={persistLead}/></section>}
        </div>
      </section>

      {showNew && <Modal close={() => setShowNew(false)} title="Add new enquiry" subtitle="Create a lead profile. You can add more details later."><form onSubmit={addLead} className="lead-form">{appError&&<div className="form-error wide">{appError}</div>}<label>Full name *<input required name="name" placeholder="Student’s full name"/></label><label>Phone number *<input required name="phone" placeholder="98XXXXXXXX"/></label><label>Email<input name="email" type="email" placeholder="name@email.com"/></label><label>Address<input name="address" placeholder="City, district"/></label><label>Education background<input name="education" placeholder="e.g. BBS · 68%"/></label><label>Passed-out institute<input name="passedOutInstitute" list="institute-options" placeholder="+2 or Bachelor’s college name"/><small>Previously added institutes will appear as suggestions.</small></label><datalist id="institute-options">{allInstitutes.map(institute=><option key={institute} value={institute}/>)}</datalist><label>Lead source<select name="source"><option>Facebook</option><option>Phone call</option><option>Walk-in</option><option>Instagram</option><option>Referral</option><option>Website</option><option>Other</option></select></label><label>Interested country<input name="country" placeholder="e.g. Australia"/></label><label>Course / subject<input name="course" placeholder="e.g. Master of IT"/></label><label className="wide">Preferred university<input name="university" placeholder="University name (optional)"/></label><label className="wide">Tags<input name="tags" placeholder="India, SRM, September intake (comma separated)"/></label><div className="form-actions"><button type="button" className="secondary" onClick={()=>setShowNew(false)}>Cancel</button><button className="primary">Create lead</button></div></form></Modal>}
      {selected && <Profile lead={selected} counsellors={counsellors} allTags={allTags} allInstitutes={allInstitutes} close={()=>setSelected(null)} update={persistLead}/>}
    </main>
  );
}
...
function TeamView() {
  const [members,setMembers]=useState<TeamMember[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [adding,setAdding]=useState(false); const [created,setCreated]=useState(""); const [resetting,setResetting]=useState<TeamMember|null>(null);
...
function Profile({lead,counsellors,allTags,allInstitutes,close,update}:{lead:Lead;counsellors:string[];allTags:string[];allInstitutes:string[];close:()=>void;update:(l:Lead,activity?:{type:string;text:string})=>void}) {
  ...
  function addTag(value:string) {
    const tag=value.trim(); if(!tag||lead.tags.some(existing=>existing.toLowerCase()===tag.toLowerCase()))return;
    update({...lead,tags:[...lead.tags,tag],updated:"Tags updated just now"},{type:"note",text:`Tag added · ${tag}`}); setTagInput(""); setFeedback(`Tag “${tag}” added.`);
  }
  function removeTag(tag:string) {
    update({...lead,tags:lead.tags.filter(item=>item!==tag),updated:"Tags updated just now"},{type:"note",text:`Tag removed · ${tag}`}); setFeedback(`Tag “${tag}” removed.`);
  }

  return <div className="drawer-backdrop" onMouseDown={e=>e.target===e.currentTarget&&close()}><aside className="profile-drawer">
    <div className="modal-head"><div><p className="eyebrow">STUDENT #{lead.id}</p></div><button aria-label="Close student profile" onClick={close}><X/></button></div>
    ...
    <div className="profile-section tag-section"><h3>Student tags</h3><div className="tag-list">{lead.tags.map(tag=><span className="student-tag" key={tag}>{tag}<button onClick={()=>removeTag(tag)} aria-label={`Remove ${tag}`}><X size={12}/></button></span>)}{lead.tags.length===0&&<small>No tags added yet.</small>}</div><form className="tag-create" onSubmit={event=>{event.preventDefault();addTag(tagInput)}}><input value={tagInput} maxLength={40} onChange={event=>setTagInput(event.target.value)} placeholder="Create or add a tag..."/><button className="primary">Add tag</button></form>{allTags.filter(tag=>!lead.tags.includes(tag)&&tag.toLowerCase().includes(tagInput.toLowerCase())).slice(0,8).length>0&&<div className="tag-suggestions"><small>Existing tags</small>{allTags.filter(tag=>!lead.tags.includes(tag)&&tag.toLowerCase().includes(tagInput.toLowerCase())).slice(0,8).map(tag=><button key={tag} onClick={()=>addTag(tag)}>+ {tag}</button>)}</div>}</div>
